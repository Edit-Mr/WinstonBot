 ...
 *
 * The table is from <https://github.com/nickdrewe/traditional-or-simplified/blob/master/TradOrSimp.js>
 *
 * @param {string} text The text to check
 * @returns {"Traditional" | "Simplified" | "Unknown"} The type of Chinese text
 */
export function detectChineseType(text) {
  ...
  const result = detectCharacters(text);

  if (result.simplifiedCharacters > result.traditionalCharacters) {
    return "Simplified";
  } else if (result.traditionalCharacters > result.simplifiedCharacters) {
    return "Traditional";
  } else {
    return "Unknown";
  }
 ...
 * @property {number} inputLength Length of the input text
 * @property {number} simplifiedCharacters Count of simplified characters
 * @property {number} traditionalCharacters Count of traditional characters
 * @property {string} detectedCharacters Detected character type
 * @property {number} detectionRate Detection rate
 */

/**
 * Analyzes a string for Chinese character types
 *
 * @param {string} text The text to analyze
 * @returns {DetectionResult} Object containing detection statistics
 */
function detectCharacters(text) {
  // Create character sets from the character maps (simplified and traditional)
  // Note: You'll need to manually create these sets from the original character maps
  ...
  if (result.simplifiedCharacters > result.traditionalCharacters) {
    ...
    result.detectionRate =
      result.traditionalCharacters > 0
        ? result.simplifiedCharacters / result.traditionalCharacters
        : result.simplifiedCharacters;
  } else if (result.traditionalCharacters > result.simplifiedCharacters) {
    result.detectedCharacters = "traditional";
    result.detectionRate =
      result.simplifiedCharacters > 0
        ? result.traditionalCharacters / result.simplifiedCharacters
        : result.traditionalCharacters;
  }
