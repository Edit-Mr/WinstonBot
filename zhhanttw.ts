 ...
 * @returns The type of Chinese text
 */
export function detectChineseType(text: string): "Traditional" | "Simplified" | "Unknown" {
	if (!text || text.trim() === "") {
		return "Unknown";
	}
...
interface DetectionResult {
	...
	inputLength: number;
	/**
	 * Count of simplified characters
	 */
	simplifiedCharacters: number;
	/**
	 * Count of traditional characters
	 */
	traditionalCharacters: number;
	/**
	 * Detected character type
	 */
 ...
 * @returns Object containing detection statistics
 */
function detectCharacters(text: string): DetectionResult {
	...
	if (result.simplifiedCharacters > result.traditionalCharacters) {
		...
		result.detectionRate = result.traditionalCharacters > 0 ? result.simplifiedCharacters / result.traditionalCharacters : result.simplifiedCharacters;
	} else if (result.traditionalCharacters > result.simplifiedCharacters) {
		result.detectedCharacters = "traditional";
		result.detectionRate = result.simplifiedCharacters > 0 ? result.traditionalCharacters / result.simplifiedCharacters : result.traditionalCharacters;
	}
